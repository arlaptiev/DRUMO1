...
Promise.all([
  ...
  function tick(time = Tone.now() - Tone.context.lookAhead) {
    console.log('tick', time, stepCounter)
  ...
  function renderPattern(regenerating = false) {
    console.log('renderPattern', state.pattern)
  ...
  function onPatternUpdated() {
    // console.log('onPatternUpdated', state.pattern)
    // stopPattern(); // DONT STOP THE PATTERN
    renderPattern();
  }
  ...
  /**
   * END CONNECTING EXTERNAL MIDI DEVICES
   */


  /**
   * INTERFACE FOR OUTSIDE SCRIPTS
   */
  playPauseDrums = function () {
    if (_.isNumber(stepCounter)) {
      stopPattern();
  ...
  generateDrums = function (seed) {
    ...
    return generatePattern(seed, state.patternLength - seed.length).then(
      result => {
        ...
        if (!hasBeenStarted) {
          console.log('Tone Transport has not been started, starting it now')
          Tone.context.resume();
          Tone.Transport.start();
          startPattern();
          hasBeenStarted = true;
  ...
  console.log('Drums are ready!')
  /**
   * END INTERFACE FOR OUTSIDE SCRIPTS
   */
  ...
  document.querySelector('.regenerate').addEventListener('click', event => {
    ...
    regenerate().then(() => {
      ...
      if (Tone.Transport.state === 'started') {
        console.log('Tone Transport is started, restarting pattern')
        // setTimeout(startPattern, 0); DONT RESTART PATTERN
      }
    });
  });
